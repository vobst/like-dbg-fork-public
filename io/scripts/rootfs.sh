...
while (("$#")); do
	...
	-h | --hostname)
		...
		;;
	-g | --git-clone)
		# comma separated list of git repos to clone
		# to .local/src
		GIT_CLONE=$2
		shift 2
		;;
	--dotfiles)
		# GitHub repo containing dotfiles and a setup script in
		# scripts/install.sh
		DOTFILES=$2
		shift 2
		;;
	--lime)
	  	# Build LiME kernel module for memory acquisition
		LIME=$2
		shift 2
		;;
	-*)
...
# 	qemu_fw_cfg: better core dumps
echo 'qemu_fw_cfg' | sudo tee -a $MNT/etc/modules >/dev/null
...
echo -en "127.0.0.1\tlocalhost $ROOTFS_NAME $HOSTNAME\n" | sudo tee $MNT/etc/hosts >/dev/null
...
echo "deb [signed-by=/usr/share/keyrings/azlux-archive-keyring.gpg] http://packages.azlux.fr/debian/ stable main" | sudo tee $MNT/etc/apt/sources.list.d/azlux.list
sudo wget -O $MNT/usr/share/keyrings/azlux-archive-keyring.gpg https://azlux.fr/repo.gpg
sudo chroot $MNT /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt update && apt install -yq broot" >/dev/null

sudo git clone "${DOTFILES}" "${MNT}/root/dotfiles"
sudo chroot $MNT /bin/bash -c "HOME=/root /root/dotfiles/scripts/install.sh && usermod -s /bin/zsh root" >/dev/null
for repo in $(echo ${GIT_CLONE} | tr ',' '\n')
do
    REPO_BASE_CHROOT="/root/.local/src/$(echo ${repo} | sed -E 's/^.*?\/([-.a-zA-Z0-9]+)$/\1/')"
    REPO_BASE="${MNT}${REPO_BASE_CHROOT}"
    sudo git clone "${repo}" "${REPO_BASE}"
    sudo chroot $MNT /bin/bash -c "cd ${REPO_BASE_CHROOT} && /usr/bin/git submodule update --init --recursive" >/dev/null
done
...
then
  ...
  OLD_BASE="$(pwd)"
  cd /tmp/lime/src
  make -C "/kernel_root" M=/tmp/lime/src
  cd ${OLD_BASE}
  sudo cp /tmp/lime/src/lime.ko ${MNT}/root/lime.ko
fi
