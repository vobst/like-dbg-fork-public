...
from lkd.memory import *
from lkd.structs import Slab, Page
...
class SearchCMD(gdb.Command):
    ...
    def lazy_init(self):
        ...
        phys_mem = VMPhysMem(pid)
        self.search_backend = PhysMemSearcher(phys_mem=phys_mem)
    ...
    def search_slab(self, address):
        slab = Slab.from_virtual(address)
        start = Page.page_to_phys(slab.address)
        length = Page.pagesize * pow(2, slab.order)
