...
def kill_session() -> None:
    ...
    tmux("selectp -t 0")
    try:
        containers = DockerRunner(
            kroot="foobar"
        ).list_running_containers()
        for c in containers:
            if c.status == "running" and next(
                (s for s in c.image.tags if "lkd_" in s), None
            ):
                c.stop(timeout=0)
...
def stage6(
    skip: bool, generic_args: dict, dbge_args: dict, dbg_args: dict
) -> None:
...
def stage5(
    skip: bool, generic_args: dict, dbge_args: dict
) -> dict[str, str]:
...
def parse_cli() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=False,
        nargs=1,
        help="Allows providing a custom 'user.ini'",
    )
    parser.add_argument(
        "--ctf",
        nargs=2,
        required=False,
        help="Expected: <kernel_image> <root_file_system>",
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument(
        ...
        "-k",
        action=argparse.BooleanOptionalAction,
        help="Completely shutdown current session",
    )
    parser.add_argument(
        "--partial",
        "-p",
    ...
    parser.add_argument(
        ...
        "-u",
        action=argparse.BooleanOptionalAction,
        help="Update all LIKE-DBG containers",
    )
...
def main():
    ...
    if args.partial and args.ctf:
        logger.error(
            "Partial runs and CTF runs are mutually exclusive!"
        )
        exit(-1)
