...
class DockerRunner:
    def __init__(self, **kwargs) -> None:
        ...
        if not kwargs.get("ctf_ctx", False):
            ...
            if not self.kernel_root:
                logger.critical(
                    f"{type(self).__name__} got invalid kernel root: '{self.kernel_root}'"
                )
                exit(-1)

    def guarantee_ssh(self, ssh_dir: Path) -> str:
        if Path(ssh_dir).exists() and "lkd.id_rsa" in os.listdir(
            ssh_dir
        ):
            ...
            if not Path(ssh_dir).exists():
                Path(ssh_dir).mkdir()
            sp.run(
                f'ssh-keygen -f {Path(ssh_dir) / "lkd.id_rsa"} -t rsa -N ""',
                shell=True,
            )
        return ssh_dir
    ...
    def init_ssh(self):
        ...
        while True:
            try:
                #                   self.ssh_conn = Connection(f"localhost", user=self.user, port=self.ssh_fwd_port, connect_kwargs={"key_filename": ".ssh/lkd.id_rsa"})
                self.ssh_conn = Connection(
                    f"localhost",
                    user=self.user,
                    port=self.ssh_fwd_port,
                    connect_kwargs={"password": "user"},
                )
            except Exception as e:
                tries += 1
                logger.error(
                    f"Failed to initialize SSH connection to {type(self).__name__}: {e}"
                )
                logger.error("Retrying in 5 seconds...")
                if tries >= 5:
                    logger.critical(
                        f"{tries} attempts failed! Exiting..."
                    )
                    exit(-1)
                time.sleep(5)
            else:
    ...
    def build_image(
        self, dockerfile=None, buildargs=None, image_tag=None
    ):
    ...
    def build_base_img(self) -> int:
        return self.build_image(
            dockerfile=self.dockerfile_base_img,
            image_tag=self.tag_base_image,
        )
    ...
    def run(self, check_existing: bool = False) -> int:
        ...
        if not self.image:
            if not self.is_base_image():
                logger.debug(
                    "Could not find 'lkd-dbg'-base image! Building it!"
                )
                self.build_base_img()
            logger.info(
                f"Building fresh image for {type(self).__name__}"
            )
            self.build_image()
    ...
    def list_running_containers(
        self,
    ) -> list[docker.client.DockerClient.containers]:
