...
class KernelBuilder(DockerRunner):
    ...
    def _run_ssh(self, cmd: str, **kwargs) -> int:
        ...
        return self.ssh_conn.run(
            f"cd {self.docker_mnt}/{self.kernel_root} && {cmd}",
            echo=True,
            warn=warn,
        ).exited
    ...
    def _apply_patches(self) -> int:
        ret = 0
        # apply patches in .mbox format generated by b4
        if (
            self.mail_patch_dir
            and Path(self.mail_patch_dir).exists()
            and self.am
        ):
            mbox = Path(f"{self.mail_patch_dir}/{self.am}")
            if not mbox.exists():
                logger.debug(f"Got {self.am=} but file does not exist")
                return 1
            logger.info(f"Patching: {mbox}")
            # git am wants operate in a git repository, so be it
            cmd = "git config --global user.email 'foo@bar.baz'"
            cmd += " && git config --global user.name 'Foo Bar'"
            cmd += " && git init && git add -A && git commit -m 'foo'"
            cmd += f" && git am ../../{self.mail_patch_dir}/{self.am}"
            logger.debug(f"Running: {cmd}")
            if self._run_ssh(cmd, warn=True) != 0:
                logger.error(f"Failed to apply patch: {self.am}")
                return 1

        # apply patches in git diff fromat
        if self.patch_dir and Path(self.patch_dir).exists():
            ...
            if patch_files:
                for pfile in patch_files:
                    ...
                    if (
                        self._run_ssh(
                            f"patch -p1 < ../../{self.patch_dir}/{pfile.name} > /dev/null",
                            warn=True,
                        )
                        != 0
                    ):
                        logger.error(
                            f"Failed to apply patch: {pfile}... Continuing anyway!"
                        )
                        ret = 1
        return ret
    ...
    def _build_mrproper(self) -> int:
        return self._run_ssh(
            f"{self.cc} ARCH={self.arch} make mrproper"
        )
    ...
    def _build_kvm_guest(self) -> int:
        return self._run_ssh(
            f"{self.cc} {self.llvm_flag} ARCH={self.arch} make kvm_guest.config"
        )
    ...
    def _get_params(self) -> str:
        ...
        if self.patch_args:
            params += " -e " + " -e ".join(self.patch_args.split())
        if params:
    ...
    def _make(self) -> int:
        ret = self._run_ssh(
            f"{self.cc} ARCH={self.arch} {self.llvm_flag} make -j$(nproc) all"
        )
        if ret != 0:
            ...
            exit(-1)
        return self._run_ssh(
            f"{self.cc} ARCH={self.arch} {self.llvm_flag} make -j$(nproc) modules"
        )
    ...
    def _add_single_mod(self, mod: Path) -> None:
        ...
        tmp = "_".join(
            re.search(r"config .*", mod_kcfg_content)[0].upper().split()
        )
    ...
    def _add_modules(self) -> None:
        ...
        if all(
            ele in [x.name for x in mods]
            for ele in ["Kconfig", "Makefile"]
        ):
    ...
    def run_container(self) -> None:
        ...
        try:
            ...
            if self.mode != "config":
                ...
                self._build_arch()
                if self.kvm:
                    logger.info("Building with kvm guest support")
            ...
            if self.arch == "x86_64":
                ...
                self.ssh_conn.run(
                    f"cd {self.docker_mnt}/{self.kernel_root}/arch/{self.arch}/boot && {cmd}",
                    echo=True,
                )
        finally:
            try:
